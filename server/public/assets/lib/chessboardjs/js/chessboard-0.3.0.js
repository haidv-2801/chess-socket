(function () {
  ...
  var ASSETS_PATH = './assets/img/chesspieces/wikipedia/';

  var BOARD_SIZE = 720;

  /**
   * validSquare(from) validSquare(to)
   * @param {object} move
   * @returns
   */
  ...
  /**
   * @param {string} square có dạng [a-h][1-8] (regex) thì hợp lệ
   * @returns
   */
  ...
  /**
   *
   * @param {string} code có dạng [bw][KQRNBP] (regex) thì hợp lệ
   * @returns
   */
  ...
  /**
   *
   * @param {object} pos key:có dạng [a-h][1-8] (regex) thì hợp lệ và value: có dạng [bw][KQRNBP] (regex) thì hợp lệ
   * @returns
   */
   ...
   * Chuyển lược đồ của bàn cờ sang dạng chuỗi
   * @param {object} obj
   * @returns ví dụ "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR"
   */
  ...
  window['ChessBoard'] =
    ...
    function (containerElOrId, cfg) {
      ...
      var ANIMATION_HAPPENING = false,
        BOARD_BORDER_SIZE = 2,
        CURRENT_ORIENTATION = 'white', //phía dưới bàn cơ
        CURRENT_POSITION = {},
        SQUARE_SIZE,
        DRAGGED_PIECE,
      ...
      function expandConfig() {
        ...
        if (
          ...
          cfg.pieceTheme = ASSETS_PATH + '/{piece}.png';
        }
       ...
       * Tạo ra id cho mỗi ô vuông có dạng [a-h][1-8]-mã GUID
       */
...
*/
      ...
      /**
       * Render ra bàn cờ
       * @param {*} orientation màu của phía quân cờ (trắng đen)
       */
      ...
      /**
       * Trả về đường dẫn đến ảnh quân cờ
       * @param {*} piece
       * @returns string
       */
      ...
      /**
       * Build vị trí mặng định của quân cờ
       * @param {*} color
       */
       ...
       */
      function drawBoard() {
        //build bàn cờ nhưng chưa có quân cờ
        boardEl.html(buildBoard(CURRENT_ORIENTATION));
        drawPositionInstant();

        //build thêm quân cờ mặc địng
        if (cfg.sparePieces === true) {
      ...
      /**
       * So sánh nếu vị trí di chuyển tới = vị trí trước đó thì dừng không làm gì
       * ngược lại cập nhật bị trí hiện tại bằng vị trí mới
       * @param {*} position
       * @returns
       */
